...
- The user-facing product and packaged application name is `Ousia`; Pi is the agent it hosts. Keep the internal `pi-gui` crate/package names, bundle identifier, data directories, log filename, environment variables, and existing PATH ownership marker stable for upgrade compatibility.
...
- Tauri macOS releases: `scripts/release-macos.sh` is the release build source of truth. It requires an explicit Developer ID identity and Apple notarization credentials, builds the `.app` and `.dmg`, waits for notarization and stapling, verifies both artifacts with Apple tooling, and emits a ZIP plus SHA-256 checksums. It derives future artifact names from the Tauri `productName` and emits a byte-identical stable-name DMG such as `Ousia_aarch64.dmg`; the README keeps the archived `Pi_aarch64.dmg` link pinned to Tauri `v0.2.6`. Never publish an artifact produced by bypassing those checks.
- Application updates: packaged Ousia builds check the signed GitHub `latest.json` feed in the background at startup and every four hours. The sidebar remains unchanged when no newer version exists or a background feed check is temporarily unavailable; background failures are recorded in the runtime log and retried on the next interval. An available update is downloaded only after explicit user action, then installed and relaunched after a second explicit action. Tauri updater archives must be signed with the private key corresponding to the public key in `src-tauri/tauri.conf.json`. Daily builds keep `createUpdaterArtifacts` disabled in the base Tauri configuration; `src-tauri/tauri.release.conf.json` enables updater artifacts only for `scripts/release-macos.sh`. That release script is the source of truth for generating and verifying `Ousia.app.tar.gz`, `Ousia.app.tar.gz.sig`, and `latest.json`; all three fixed-name files must be attached to the matching GitHub release without renaming. User-initiated download, verification, installation, and relaunch failures must be visible in the UI and runtime log. The updater private key defaults to `~/.tauri/pi-updater.key`, or `TAURI_SIGNING_PRIVATE_KEY_PATH` when set. When `TAURI_SIGNING_PRIVATE_KEY` is absent and that file exists, load it automatically into the environment for desktop validation and release work; use the existing `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`, defaulting to an empty value when it is unset. Never print, decode, preview, log, or include the private-key contents in a command result or conversation, and never ask the user to paste the key. If the resolved key file is absent or signing rejects the available password, fail clearly with only the path/status and report the blocker.
...
if [[ -z "${TAURI_SIGNING_PRIVATE_KEY:-}" ]]; then
  if [[ ! -f "$updater_key_path" ]]; then
    print -u2 -- "Tauri updater signing key is unavailable: $updater_key_path"
    exit 1
  fi
...
The key-loading command must run without shell tracing and without printing the environment. Do not replace this signed validation with `createUpdaterArtifacts: false` when the repository key is available. Use `scripts/release-macos.sh` for an actual release because it additionally enforces Developer ID signing, notarization, stapling, artifact verification, and checksum generation.
